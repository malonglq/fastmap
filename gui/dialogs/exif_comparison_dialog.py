...
class FieldPreviewWorker(QThread):
    """字段预览工作线程"""
    ...
    def run(self):
        try:
            ...
            self.finished.emit(field_info)
        except Exception as e:
            self.error.emit(str(e))


class ExifComparisonDialog(QDialog):
    """
    EXIF对比配置对话框
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("EXIF对比分析配置")
        self.setModal(True)
        self.resize(800, 600)
    ...
    def setup_ui(self):
        ...
        # 新增：图片分类导出
        self.export_btn = QPushButton("图片分类导出…")
    ...
    def create_field_configuration_tab(self) -> QWidget:
        ...
        self.select_core_btn = QPushButton("选择核心字段")
    ...
    def load_test_file_info(self, file_path: str):
        ...
        try:
            ...
            # 创建工作线程
            self.test_worker = FieldPreviewWorker(file_path, self.generator)
            self.test_worker.finished.connect(self.on_test_field_info_loaded)
            self.test_worker.error.connect(self.on_field_info_error)
    ...
    def load_reference_file_info(self, file_path: str):
        ...
        try:
            ...
            # 创建工作线程
            self.reference_worker = FieldPreviewWorker(file_path, self.generator)
            self.reference_worker.finished.connect(self.on_reference_field_info_loaded)
            self.reference_worker.error.connect(self.on_field_info_error)
    ...
    def on_test_field_info_loaded(self, field_info: Dict[str, Any]):
        """测试机字段信息加载完成"""
    ...
    def on_reference_field_info_loaded(self, field_info: Dict[str, Any]):
        """对比机字段信息加载完成"""
    ...
    def on_field_info_error(self, error_msg: str):
        """字段信息加载错误"""
    ...
    def populate_field_table(self):
        ...
        try:
            ...
            common_fields = test_numeric_fields.intersection(reference_numeric_fields)
            ...
            # 填充表格
            for row, field_name in enumerate(sorted(common_fields)):
    ...
    def select_core_fields(self):
        """选择核心字段"""
        core_fields = self.generator.core_fields
        ...
        for row in range(self.field_table.rowCount()):
            ...
            if field_name_item and checkbox:
                ...
                checkbox.setChecked(field_name in core_fields)
...
"""
    ...
    def open_image_export_dialog(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "错误", f"无法打开导出对话框:\n{e}")
